...
class ProductiveStageService {
    ...
    async assignInstructors(id, data, performedBy) {
        ...
        const oldInstructors = {
            followupInstructor: ep.followupInstructor?.toString(),
            technicalInstructor: ep.technicalInstructor?.toString(),
            projectInstructor: ep.projectInstructor?.toString()
        };

        const removedInstructorIds = [];
        const newlyAssignedInstructorIds = [];
        
        for (const role of ['followupInstructor', 'technicalInstructor', 'projectInstructor']) {
            const oldId = oldInstructors[role];
            const newId = updates[role];
            
            if (newId && oldId !== newId) {
                newlyAssignedInstructorIds.push(newId);
                if (oldId) removedInstructorIds.push(oldId);
            }
        }
        ...
        // Populate para obtener datos del aprendiz y empresa para la notificación
        await ep.populate([
            { path: "apprentice", select: "fullName program enrollmentNumber" },
        ...
        const modalityName = modalityNames[ep.modality] || ep.modality;
        const companyName = ep.companySnapshot?.companyName || ep.company?.name || 'Sin empresa asignada';

        const reason = data.reason || 'Reasignación administrativa';

        // Notificar a los instructores removidos
        if (removedInstructorIds.length > 0) {
            const removedMessage = `
                El aprendiz <strong>${ep.apprentice?.fullName}</strong> (Ficha: ${ep.apprentice?.enrollmentNumber || 'N/A'}) ha sido reasignado a otro instructor.<br><br>
                <strong>Motivo del cambio:</strong> ${reason}
            `;
            await notificationService.send({
                type: "APPRENTICE_REASSIGNED",
                recipients: removedInstructorIds,
                title: "Aprendiz removido por reasignación",
                message: removedMessage,
                metadata: { entity: "ProductiveStage", entityId: ep._id }
            });
        }

        // Notificar a los instructores recién asignados
        if (newlyAssignedInstructorIds.length > 0) {
            const addedMessage = `
                Se te ha asignado un nuevo aprendiz para etapa productiva${removedInstructorIds.length > 0 ? ' (por transferencia)' : ''}.<br><br>
                <strong>Nombre del aprendiz:</strong> ${ep.apprentice?.fullName}<br>
                <strong>Programa:</strong> ${ep.apprentice?.program || 'N/A'}<br>
                <strong>Ficha:</strong> ${ep.apprentice?.enrollmentNumber || 'N/A'}<br>
                <strong>Modalidad:</strong> ${modalityName}<br>
                <strong>Fecha de asignación:</strong> ${new Date().toLocaleDateString('es-CO')}<br>
                <strong>Información de la empresa:</strong> ${companyName}<br>
                ${removedInstructorIds.length > 0 ? `<strong>Motivo del cambio:</strong> ${reason}` : ''}
            `;
            await notificationService.send({
                type: "APPRENTICE_ASSIGNED",
                recipients: newlyAssignedInstructorIds,
