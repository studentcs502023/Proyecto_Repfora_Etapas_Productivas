<template>
  <div class="certification-container q-pa-md">
    <div class="row items-center q-mb-md">
      <div class="col">
        <h2 class="text-h4 text-black text-weight-bold q-my-none">Certificación Final</h2>
        ...
        <div class="col-12 col-md-4" v-for="docType in requiredDocumentTypes" :key="docType.value">
          <q-card flat bordered class="full-height flex column">
            <q-card-section class="bg-grey-2 q-pb-xs">
              <div class="text-subtitle1 text-weight-bold text-black">{{ docType.label }}</div>
            </q-card-section>
...
async function loadData() {
  loading.value = true;
  try {
    // Interceptor returns { success, message, data: { eps: [...] } }
    ...
    if (ep.value && ep.value._id) {
      // getEPStatus returns { success, message, data: <status_object> }
      const statusRes = await documentService.getEPStatus(ep.value._id);
      epStatus.value = statusRes.data || null;
    }
  } catch (error) {
    console.error(error);
