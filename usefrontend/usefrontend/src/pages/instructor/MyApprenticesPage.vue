<template>
  <div class="my-apprentices-container q-pa-md">
    ...
    <!-- Tabla -->
    <q-card flat bordered v-if="filteredEps.length > 0 || loading">
      <q-table
        :rows="filteredEps"
        :columns="columns"
        :loading="loading"
    ...
    <!-- Estado Vacío -->
    <q-card flat bordered v-else class="q-pa-xl text-center bg-grey-1 q-mt-md">
      <q-card-section>
...
onMounted(() => {
  fetchApprentices();
  // Reactividad: Polling para actualizar lista automáticamente
  pollingInterval = setInterval(() => {
    fetchApprentices(false);
  }, 10000);
});
...
async function fetchApprentices(showLoading = true) {
  ...
  try {
    ...
    console.error(error);
    if (showLoading) $q.notify({ type: 'negative', message: error.message || 'Error al cargar aprendices.' });
  } finally {
