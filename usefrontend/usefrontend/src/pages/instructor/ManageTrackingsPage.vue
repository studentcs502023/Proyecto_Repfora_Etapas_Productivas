<template>
  <div class="manage-trackings-container q-pa-md">
      ...
      >
        ...
        <template v-slot:body-cell-extraordinario="props">
          <q-td :props="props" class="text-center">
            <template v-if="props.row.isExtraordinary">
              <!-- Chip de estado del extraordinario -->
              <q-chip
                v-if="props.row.extraordinaryStatus === 'Pendiente'"
                color="orange" text-color="white" icon="schedule" dense size="sm"
                label="Pendiente"
              />
              <q-chip
                v-else-if="props.row.extraordinaryStatus === 'Ejecutado'"
                color="positive" text-color="white" icon="task_alt" dense size="sm"
                label="Ejecutado"
              />
              <q-chip
                v-else-if="props.row.extraordinaryStatus === 'Cobrado'"
                color="purple" text-color="white" icon="payments" dense size="sm"
                label="Cobrado"
              />
              <!-- Esperando aprobación de admin -->
              <q-chip
                v-else
                color="grey-5" text-color="white" icon="hourglass_empty" dense size="sm"
                label="Sin autorizar"
              />
            </template>
        ...
        <template v-slot:body-cell-actions="props">
          <q-td :props="props" class="q-gutter-xs">
            <!-- Botón Seguimiento Extraordinario -->
            <q-btn
              v-if="props.row.isExtraordinary && props.row.approvedByAdmin && props.row.extraordinaryStatus === 'Pendiente'"
              size="sm" color="warning" text-color="black" icon="star" unelevated
              @click="openExtraordinaryDialog(props.row)"
            >
              <q-tooltip>Ejecutar Seguimiento Extraordinario</q-tooltip>
            </q-btn>
    ...
    <!-- ================================================ -->
    <!-- Modal: Seguimiento Extraordinario                -->
    <!-- ================================================ -->
    <q-dialog v-model="showExtraordinaryDialog" persistent>
      <q-card style="width: 560px; max-width: 95vw;">
        <q-card-section class="bg-warning text-black row items-center">
          <q-icon name="star" class="q-mr-sm" size="sm" />
          <div class="text-h6">Ejecutar Seguimiento Extraordinario</div>
          <q-space />
          <q-btn flat round dense icon="close" color="black" v-close-popup />
        </q-card-section>

        <q-card-section v-if="extraDialogTracking" class="q-pa-md q-gutter-sm">
          <!-- Info del Aprendiz (sólo lectura) -->
          <div class="text-subtitle2 text-grey-7 q-mb-xs">Información del Aprendiz</div>
          <q-card flat bordered class="q-pa-sm q-mb-md bg-grey-1">
            <div class="row q-col-gutter-sm">
              <div class="col-12 col-sm-6">
                <div class="text-caption text-grey-6">Nombre Completo</div>
                <div class="text-weight-medium">{{ extraDialogTracking.apprentice?.fullName || '—' }}</div>
              </div>
              <div class="col-12 col-sm-6">
                <div class="text-caption text-grey-6">Documento</div>
                <div class="text-weight-medium">{{ extraDialogTracking.apprentice?.nationalId || '—' }}</div>
              </div>
              <div class="col-12 col-sm-6">
                <div class="text-caption text-grey-6">Ficha</div>
                <div class="text-weight-medium">{{ extraDialogTracking.apprentice?.enrollmentNumber || '—' }}</div>
              </div>
              <div class="col-12 col-sm-6">
                <div class="text-caption text-grey-6">Programa</div>
                <div class="text-weight-medium">{{ extraDialogTracking.apprentice?.program || '—' }}</div>
              </div>
              <div class="col-12">
                <div class="text-caption text-grey-6">Modalidad</div>
                <div class="text-weight-medium">{{ extraDialogTracking.productiveStage?.modality || '—' }}</div>
              </div>
            </div>
          </q-card>

          <!-- Carga de archivo PDF -->
          <div class="text-subtitle2 text-grey-7 q-mb-xs">Soporte del Seguimiento (PDF)</div>
          <q-file
            v-model="extraFile"
            ...
            class="q-mb-sm"
            @rejected="onExtraFileRejected"
          >
            <template v-slot:prepend><q-icon name="picture_as_pdf" color="negative" /></template>
          </q-file>

          <q-banner v-if="extraAiVerified" class="bg-green-1 text-positive rounded-borders q-mb-sm text-caption">
            <template v-slot:avatar><q-icon name="verified" /></template>
            Documento verificado correctamente. Puede guardar.
          </q-banner>

          <!-- Botón verificar con IA -->
          <q-btn
            outline color="primary" icon="psychology" label="Verificar firmas con IA"
            :loading="extraVerifying"
            :disable="!extraFile"
        ...
        <q-card-actions align="right" class="q-pa-md">
          <q-btn flat label="Cancelar" color="grey" v-close-popup />
          <q-btn
            color="warning" text-color="black" icon="save" label="Guardar y Ejecutar"
            :loading="extraSaving"
            :disable="!extraFile || !extraAiVerified"
            @click="saveExtraordinary"
          />
        </q-card-actions>
      </q-card>
    </q-dialog>
...
const columns = [
  ...
  { name: 'extraordinario', label: 'Extraordinario', field: 'extraordinaryStatus', align: 'center' },
...
// Estado para el nuevo dialog de extraordinario
const showExtraordinaryDialog = ref(false);
const extraDialogTracking = ref(null);
const extraFile = ref(null);
const extraAiVerified = ref(false);
const extraVerifying = ref(false);
...
// ───────────────────────────────────────────────
// Dialog de Seguimiento Extraordinario
// ───────────────────────────────────────────────
function openExtraordinaryDialog(tracking) {
  extraDialogTracking.value = tracking;
  extraFile.value = null;
  extraAiVerified.value = false;
  showExtraordinaryDialog.value = true;
}

function onExtraFileRejected(entries) {
  const reason = entries[0]?.failedPropValidation;
  if (reason === 'accept') {
    $q.notify({ type: 'warning', message: 'El archivo debe estar en formato PDF y contener las firmas requeridas.' });
  } else if (reason === 'max-file-size') {
    $q.notify({ type: 'warning', message: 'El archivo no debe superar los 10 MB.' });
  }
}

async function verifyExtraAI() {
  if (!extraFile.value) return;
  // Verificar que sea PDF antes de llamar al endpoint
  if (extraFile.value.type !== 'application/pdf') {
    $q.notify({ type: 'warning', message: 'El archivo debe estar en formato PDF y contener las firmas requeridas.' });
    return;
  }
  extraVerifying.value = true;
  try {
    // Llamada al endpoint mock de IA (reutiliza el mismo que el wizard)
    await trackingService.verifyAI ? trackingService.verifyAI() : Promise.resolve();
    extraAiVerified.value = true;
    $q.notify({ type: 'positive', message: 'Verificación de firmas exitosa.' });
  } catch {
    $q.notify({ type: 'negative', message: 'Error al verificar el documento. Intenta nuevamente.' });
  } finally {
    extraVerifying.value = false;
  }
}

async function saveExtraordinary() {
  if (!extraFile.value || !extraAiVerified.value) {
    $q.notify({ type: 'warning', message: 'El archivo debe estar en formato PDF y contener las firmas requeridas.' });
    return;
  }
  ...
  try {
    ...
    await trackingService.uploadExtraordinaryPDF(extraDialogTracking.value._id, fd);
    // 2. Ejecutar el seguimiento
    await trackingService.execute(extraDialogTracking.value._id);
    $q.notify({ type: 'positive', message: 'Seguimiento extraordinario ejecutado correctamente.' });
    showExtraordinaryDialog.value = false;
    fetchTrackings();
  } catch (error) {
    $q.notify({ type: 'negative', message: error.message || 'Error al guardar el seguimiento extraordinario.' });
  } finally {
